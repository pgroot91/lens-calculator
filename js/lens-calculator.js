function MakeArray(n) {                 
	...
	function compute_it_hoogte(form) {
        ...
        if (isNaN(result2)){
           alert( object_name.message2);document.hoogte.objectafstand.focus();}
        if (isNaN(result3)){
           alert( object_name.message31);document.hoogte.objecthoogte.focus();}
		// Bereken nu het totaal.       
	
	if (result1!=0 && !isNaN(result2) && !isNaN(result3)) {
		document.totalhoogte.output.value = Math.round(parseFloat(document.hoogte.objectafstand.value) * 
		parseFloat(SensorHoogtes[result1 - 1]) /
	  	parseFloat(document.hoogte.objecthoogte.value));	  					      
	}
	else {
	  document.totalhoogte.output.value = ( object_name.nnb);  
	}
}
...
function compute_it_breedte(form) {
        ...
        if (result1==0){
           alert( object_name.message1);}
        if (isNaN(result2)){
           alert( object_name.message2);document.breedte.objectafstand.focus();}
        if (isNaN(result3)){
           alert( object_name.message32);document.breedte.objectbreedte.focus();}
		// Bereken nu het totaal.       
		if (result1!=0 && !isNaN(result2) && !isNaN(result3)) {
		document.totalbreedte.output.value = Math.round(parseFloat(document.breedte.objectafstand.value) * 
		parseFloat(SensorBreedtes[result1 - 1]) /
	  	parseFloat(document.breedte.objectbreedte.value));
	}
	else {
	  document.totalbreedte.output.value = ( object_name.nnb);
	}
}